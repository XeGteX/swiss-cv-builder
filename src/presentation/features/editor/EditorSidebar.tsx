...
export const EditorSidebar: React.FC = React.memo(() => {
    ...
    // Get visible tabs: show 2 by default (1 row), expand to all 6 on hover
    const getVisibleTabs = () => {
        ...
        // If active tab is in first 2, return first 2
        if (activeTabIndex < 2) {
            return defaultTabs;
        }

        // Otherwise, replace second tab with active tab
            ...
            >
                ...
                >
                    <AnimatePresence mode="popLayout">
                        {(isExpanded ? ALL_TABS : visibleTabs).map((tab) => {
                            const Icon = tab.icon;
