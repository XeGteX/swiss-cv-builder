/**
 * NEXAL2 Block System - IconSet
 * 
 * Vector SVG paths for premium icons (phone, email, location, link).
 * Fill-only paths for easy recoloring via CSS/tokens.
 */
...
/**
 * Premium icon set - fill-only SVG paths
 * Designed at 24x24 viewBox for crisp rendering at 12-16pt
 */
export const ICON_SET: Record<string, IconDef> = {
    phone: {
        viewBox: '0 0 24 24',
        paths: [
...
/**
 * Get icon field type from contact field name
 */
export function getContactIcon(field: string): keyof typeof ICON_SET | null {
    switch (field) {
        case 'email':
            return 'email';
        case 'phone':
